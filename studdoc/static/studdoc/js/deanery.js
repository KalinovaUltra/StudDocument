...
export function initDeanery() {
    requestsData = window.DEANERY_DATA || [];
            ...
            <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 10px;">
                ...
                <div style="display: flex; gap: 10px;">
                    ...
                    <select class="status-select" data-id="${req.id}" style="padding: 5px; border-radius: 4px;">
                        <option value="pending" ${req.status === 'pending' ? 'selected' : ''}>На рассмотрении в деканате</option>
                        <option value="approved" ${req.status === 'approved' ? 'selected' : ''}>Одобрено деканатом</option>
                        <option value="rejected" ${req.status === 'rejected' ? 'selected' : ''}>Отклонено деканатом</option>
                        <option value="in_profcom" class="send-option" ${req.status === 'in_profcom' ? 'selected' : ''}>📤 Отправить в профком</option>
                    </select>
                </div>
            </div>
        </div>
